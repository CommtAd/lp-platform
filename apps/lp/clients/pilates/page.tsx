...
/**
 * パターンBのテンプレはCTAがオレンジ（#FF8A50）前提だが、コミットアドforピラティスの
 * ブランドCTAは緑。既存クリエイティブ・Meta広告と色を揃えるため、この2つの
 * グラデーションだけテンプレから意図的に差分を持たせている。
 */
const ctaGradAngle = (cta: string) => `linear-gradient(135deg, ${cta} 0%, #6FD46B 100%)`;
const FV_GRADIENT = "linear-gradient(160deg,#4A2F47 0%,#3A2338 55%,#2A1928 100%)";
...
export default function Page() {
  ...
  return (
    <LPShell clientSlug={c.slug} fallback={{ name: c.meta.title, status: c.status }}>
      <div style={{ ...vars, background: "#F3EEF2", minHeight: "100vh" }} className="text-[var(--navy)]">
        <LPCanvas className="bg-white" boxShadow="0 0 60px rgba(74,47,71,0.16)">
          {/* ── header ── */}
          <header className="sticky top-0 z-40 flex items-center justify-between gap-3 border-b border-slate-100 bg-white/95 px-5 py-4 backdrop-blur">
            ...
            <a
              href="#form"
              className="shrink-0 rounded-full bg-[var(--cta)] px-4 py-2.5 text-[12px] font-bold text-white shadow-[0_4px_14px_rgba(57,186,54,0.35)]"
            >
              {c.header.ctaText}
            </a>
          </header>
          ...
          {/* ── FV ── */}
          <section className="px-5 py-12" style={{ background: FV_GRADIENT }}>
            ...
            <a
              href="#form"
              className="mt-5 flex h-14 items-center justify-center gap-2 rounded-full text-[14.5px] font-bold text-white shadow-[0_10px_26px_rgba(57,186,54,0.45)]"
          ...
          <section className="relative overflow-hidden px-5 py-16 text-center text-white">
            ...
            <div className="relative">
              ...
              <a
                href="#form"
                className="mt-7 inline-flex h-14 items-center justify-center gap-2 rounded-full px-7 text-[14.5px] font-bold text-white shadow-[0_10px_26px_rgba(57,186,54,0.45)]"
                style={{ background: ctaGradAngle(c.cta) }}
              >
          ...
          <section id="form" className="bg-white px-5 py-12">
            <SimulationSection
              ...
              accent={c.accent}
              cta={c.cta}
              ctaGradient={ctaGradAngle(c.cta)}
