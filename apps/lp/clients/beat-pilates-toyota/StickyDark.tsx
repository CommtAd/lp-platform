...
export interface StickyDarkProps {
  offers: ReactNode[];
  buttonText: string;
  /** In-page hash ("#reserve") — controls hide-on-scroll when the target is in view. */
  anchor: string;
  /** Actual link target. When it's an external URL, the bar opens it in a new tab. */
  href?: string;
  gradient: string;
  showAfter?: number;
}
...
export default function StickyDark({
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // Only in-page anchors ("#…") gate visibility; external links keep the bar shown.
