...
/* All CTAs link to the external Hacomono 予約 page (opens a new tab). */
  ...
  return (
    <LPShell clientSlug={c.slug} fallback={{ name: c.meta.title, status: c.status }}>
      <div style={{ fontFamily: fontSans, background: bg, minHeight: "100vh", color: "#FFFFFF" }}>
        <LPCanvas background={bg} style={{ color: "#FFFFFF", position: "relative" }}>
          {/* ── header ── */}
          ...
          <section style={{ padding: "34px 0 52px", background: "#0D0D14" }}>
            ...
            >
              {c.voices.items.map((v, i) => {
                const accent = i === 1 ? violet : i === 2 ? blue : pink;
                return (
                  <div key={i} style={{ flex: "none", width: 288, maxWidth: Math.round(DESIGN_WIDTH * 0.84), scrollSnapAlign: "center", background: panel, borderRadius: 18, border: `1px solid ${accent}55`, boxShadow: `0 0 20px ${accent}33`, padding: "24px 22px 26px", display: "flex", flexDirection: "column" }}>
          ...
          {/* ── reserve（外部 Hacomono 予約へ。LPFormは未使用） ── */}
          <section id="reserve" style={{ padding: "34px 22px 30px", background: "linear-gradient(160deg, #0B1030 0%, #171436 55%, #2A1A47 100%)" }}>
            ...
            <p style={{ textAlign: "center", fontSize: 12.5, lineHeight: 1.9, color: textDim, margin: "16px 0 0" }}>{nl(c.reserve.lead)}</p>
            ...
            <a
              ...
              style={{
                ...
                whiteSpace: "nowrap",
                boxShadow: "0 10px 26px rgba(255,61,147,0.32)",
                animation: "beatCtaPulse 1.8s ease-in-out infinite",
              }}
            >
              {c.reserve.ctaText}
              <span style={{ fontSize: 15 }}>↗</span>
            </a>
            <p style={{ textAlign: "center", fontSize: 9.5, letterSpacing: "-0.01em", lineHeight: 1.7, color: textDim2, margin: "12px 0 0" }}>{c.reserve.note}</p>
          </section>
