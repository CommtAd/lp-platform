...
/** Sticky footer CTA for the Beê white/gold template. */
export default function Sticky({
  ...
  // 外部URLの場合はページ内アンカーが無いので querySelector を回避し常時表示扱い。
  ...
  useEffect(() => {
    ...
    // background (globals.css uses pattern A's cream) so the padding doesn't
    // show as a mismatched strip under this white/gold LP.
    const prevPad = document.body.style.paddingBottom;
    const prevBg = document.body.style.background;
    document.body.style.paddingBottom = "104px";
    document.body.style.background = "#EFE8DA";
    return () => {
    ...
    >
      <div
        style={{
          ...
          borderRadius: 16,
          boxShadow: "0 8px 26px rgba(120,95,40,0.2)",
          pointerEvents: "auto",
          display: "flex",
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14 }}>
          {offers.map((o, i) => (
            <span key={o.label} style={{ display: "contents" }}>
              {i > 0 && <span style={{ width: 1, height: 14, background: "rgba(120,95,40,0.2)" }} />}
              <span style={{ display: "flex", alignItems: "baseline", gap: 5 }}>
                <span style={{ fontSize: 11, color: "#7A7364" }}>{o.label}</span>
