...
/**
 * WAKATA PHYSIO STUDIO（WPS / テスト枠）— 指示書の18セクション構成。
 *
 * レイアウトは `seren-pilates/page.tsx` と同一コピー。差分は `config.ts` と
 * 配色定数、および二重価格の下に条件の注記（`campaign.note`）を1行足した点のみ。
 * テンプレ側を直したときは、こちらにも同じ修正を反映すること。
 *
 * 守っているルール:
 ...
 *
 * 予約は公式サイトと同じ Square へ外部遷移するためページ内フォームを持たない。
 * `scripts/check-rules.ts` の FORM_EXEMPT に `wps-test` を登録済み。
 */
...
const accent = c.accent; // #006C38 ロゴのブランドグリーンを暗くした深緑。白地 6.6:1
const accentMid = "#009944"; // ロゴのブランドグリーン。価格・チェックの強調（白地 3.7:1）
const base = "#FCFBF8"; // 基本の地（ほぼ白）
const cream = "#F6F0E5"; // 淡色の地（公式サイトが敷いている生成りそのまま）
const creamDeep = "#EDE3CE";
...
/*
   ...
   白地でも沈まない深緑のグラデーションで固定する（追従フッターと共用）。
*/
const ctaGrad = `linear-gradient(135deg, ${shade(accent, 0.12)} 0%, ${shade(accent, -0.18)} 100%)`;
const ink = "#322B2B"; // 公式サイトの本文色そのまま
const inkSoft = "#5C5252";
const inkMute = "#918989";
    ...
    <div style={{ marginTop: 30 }}>
      <a
        ...
        style={{
          ...
          borderRadius: 999,
          boxShadow: "0 10px 22px rgba(0,50,25,0.30)",
        }}
      >
        {c.reserve.label}
    ...
    <>
      ...
      */}
      <div style={{ marginTop: compact ? 18 : 26, textAlign: "center" }}>
        ...
        {/*
          「実質0円」は当日入会が条件。金額のすぐ下に条件を置かないと
          打ち消し線つきの二重価格だけが目に残るため、必ず対で表示する。
        */}
        <p
          style={{
            fontSize: 11.5,
...
export default function Page() {
  return (
    <LPShell clientSlug={c.slug} fallback={{ name: c.meta.title, status: c.status }}>
      ...
      >
        <LPCanvas style={{ background: base }} boxShadow="0 0 60px rgba(45,55,45,0.16)">
          {/* ── ① ヘッダー ── */}
          <header
            style={{
              display: "flex",
          ...
          >
            <div style={{ lineHeight: 1.25 }}>
              <div
                style={{
                  fontFamily: fontMincho,
                  /* ブランド名が20文字と長いので、seren より2段階小さくして1行に収める
                     （18pxのままだと2行に折り返し、右の店舗名も1文字だけ落ちる）。 */
                  fontSize: 14,
                  fontWeight: 600,
                  letterSpacing: "0.04em",
                  color: accent,
          ...
          <div
            style={{
              ...
              padding: "14px 18px",
              boxShadow: "0 3px 10px rgba(45,55,45,0.18)",
            }}
          >
            <span
              style={{
                ...
                borderRadius: 8,
                padding: "7px 13px",
                boxShadow: "0 2px 6px rgba(0,45,22,0.26)",
              }}
            >
            ...
            <div style={{ flex: 1, display: "flex", justifyContent: "center" }}>
              <div
                style={{
                  ...
                  color: "#FFFFFF",
                  textShadow: "0 1px 5px rgba(10,32,20,0.5)",
                  lineHeight: 1.2,
                  textAlign: "center",
                }}
              >
          ...
          {c.supervision && (
            <div
              style={{
                ...
                color: ink,
                padding: "8px 18px",
                boxShadow: "0 2px 6px rgba(45,55,45,0.08)",
              }}
            >
          ...
          */}
          <section style={{ position: "relative", height: 480, overflow: "hidden", background: accent }}>
            ...
            <div
              style={{
                ...
                inset: 0,
                background:
                  "linear-gradient(to bottom, rgba(10,32,20,0.06) 0%, rgba(10,32,20,0) 34%, rgba(10,32,20,0.42) 100%)",
                pointerEvents: "none",
              }}
            />
              ...
              {c.worry.items.map((item) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 12,
                    padding: "14px 13px",
                    boxShadow: "0 4px 12px rgba(45,55,45,0.06)",
                  }}
                >
                  <CheckIcon />
                  <span style={{ fontSize: 14, lineHeight: 1.7, color: "#4A4442" }}>{nl(item)}</span>
                </div>
              ))}
            </div>
            {/*
          ...
          <section style={{ background: base, padding: "10px 26px 56px" }}>
            ...
            <ImageSlot
              ...
              objectPosition={c.posture.photo.position ?? "center"}
              radius={16}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                {c.posture.items.map((item) => (
                  <div key={item} style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                    <CheckIcon />
                    <span style={{ fontSize: 14, lineHeight: 1.7, color: "#4A4442" }}>{item}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <section style={{ background: base, padding: "54px 26px" }}>
            ...
            >
              {c.beginner.items.map((item) => (
                ...
                >
                  <CheckIcon />
                  <span style={{ fontSize: 13, lineHeight: 1.7, color: "#4A4442" }}>{item}</span>
                </div>
              ))}
            </div>
          </section>
              ...
              {c.stores.items.map((store) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 16,
                    overflow: "hidden",
                    boxShadow: "0 6px 16px rgba(45,55,45,0.10)",
                  }}
                >
                  <ImageSlot
                    src={store.img.src}
                    placeholder={store.img.placeholder}
                    objectPosition={store.img.position ?? "center"}
                  ...
                  <div style={{ padding: 20 }}>
                    ...
                    <p style={{ fontSize: 14, lineHeight: 1.9, color: inkSoft, margin: 0 }}>
                      {nl(store.address)}
                      <br />
                      <span style={{ color: accent, fontWeight: 700 }}>{nl(store.hours)}</span>
                      <br />
                      {store.closed}
                    </p>
