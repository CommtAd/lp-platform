            ...
            {c.reasons.items.map((item) => (
              <div key={item.num} style={{ marginTop: 34 }}>
                <div style={{ position: "relative" }}>
                  <ImageSlot src={item.img.src} placeholder={item.img.placeholder} objectPosition={item.img.position ?? "center"} radius={16} style={item.imgAspect ? { width: "100%", aspectRatio: item.imgAspect, background: "#E4DCCB" } : { width: "100%", height: 190, background: "#E4DCCB" }} />
                  <span
                    style={{
                      position: "absolute",
                      top: -12,
