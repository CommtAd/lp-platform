...
/*
 * ARCS uses the site's brand orange (#F08D1B) as the main colour instead of
 * pattern A's navy. Orange is far lighter than navy, so the single accent has
 * to be split by role or text stops being readable:
 *   accent     — the pure brand orange, on the small filled surfaces that
 *                carry short bold white labels (FV cards, the eyebrow badge,
 *                reason diamonds, step circles). This mirrors the site, which
 *                pairs #f08d1b with white text.
 *   accentDeep — darkened to ~5:1 against white. Used for orange TEXT on
 *                cream (headings, tags, hours), where the pure orange drops to
 *                ~2.3:1, and for the full-bleed sections that carry paragraphs
 *                of white body copy. Darkening much past this turns brown, so
 *                the section gradient keeps a narrower range than the navy one.
 */
const accent = config.accent;
const accentDeep = shade(accent, -0.32);
const accentInk = accentDeep;
const navyGrad = `linear-gradient(158deg, ${shade(accentDeep, 0.06)} 0%, ${accentDeep} 52%, ${shade(accentDeep, -0.3)} 100%)`;
...
/*
 * ARCS's trial is discounted but not free (5,500円 → 1,980円), unlike the
 * pattern-A default of a 0円 trial. Shrink the big gold figure as the number
 * gets longer so "1,980円" still fits beside the struck-through regular price.
 */
const trialNow = config.offer.trialNow ?? "0";
const trialNowSize = trialNow.length <= 1 ? 92 : trialNow.length <= 3 ? 72 : 48;
...
function SectionHeading({
  ...
  const color = variant === "white" ? "#FFFFFF" : accentInk;
  const rule = variant === "white" ? "rgba(255,255,255,0.55)" : accentInk;
  return (
      ...
      >
        ...
        >
          ...
          >
            ...
            <span style={{ fontWeight: 700, fontSize: 16, lineHeight: 1, color: accentInk }}>
              {c.achievement.num}
            </span>
          ...
          <section style={{ background: "#FCFBF7", padding: "54px 24px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14, marginTop: 26 }}>
              ...
              <div style={{ flex: "none", whiteSpace: "nowrap", fontFamily: fontMincho, fontWeight: 700, fontSize: trialNowSize, lineHeight: 0.9, letterSpacing: "0.02em", background: goldGrad, WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>
                {trialNow}<span style={{ fontSize: Math.round(trialNowSize * 0.52) }}>円</span>
              </div>
            </div>
                ...
                <div key={i} style={{ background: "#FCFBF7", borderRadius: 16, overflow: "hidden", boxShadow: "0 6px 16px rgba(70,72,60,0.10)" }}>
                  ...
                  <div style={{ padding: 20 }}>
                    ...
                    <p style={{ fontSize: 12, lineHeight: 1.9, color: "#62655B", margin: "8px 0 0" }}>
                      {store.address}
                      <br />
                      <span style={{ color: accentInk, fontWeight: 700 }}>{store.hours}</span>
                      {nl(store.route)}
                    </p>
                  </div>
                </div>
