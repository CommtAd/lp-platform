...
ヘッダー → FV（会場写真＋キャッチ＋訴求バンド＋オファーチップ）→ ブランド紹介 → 限定特典（成約特典）
...
（`brand` / `grandOffer` / `schedule` / `recommend` / `gallery` / `reasons` / `plan` /
`voices` / `faq` / `overview`）。式場によって出せる情報が大きく違うため、揃わない項目を
